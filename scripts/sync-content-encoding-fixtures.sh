#!/usr/bin/env bash
# sync-content-encoding-fixtures.sh — refresh the vendored copy of the shared
# content-encoding fixture corpus under
# crates/tapesctl/vendor/tapes-content-encoding-fixtures/.
#
# The corpus is authored in the `tapes` repository at fixtures/content-encoding/
...
#
# Usage:
#   ./scripts/sync-content-encoding-fixtures.sh <path-to-tapes-checkout>
#   ./scripts/sync-content-encoding-fixtures.sh --check <path-to-tapes-checkout>
#
...
VENDOR_DIR="$REPO_ROOT/crates/tapesctl/vendor/tapes-content-encoding-fixtures"
...
src_dir="$checkout_path/fixtures/content-encoding"
if [[ ! -d "$src_dir/cases" ]]; then
    echo "error: no content-encoding fixtures at $src_dir/cases" >&2
...
upstream_sha="$(git -C "$checkout_path" log -1 --format=%H -- fixtures/content-encoding 2>/dev/null || true)"
...
if $check_only; then
    ...
    if [[ $status -eq 0 ]]; then
        echo "vendored content-encoding fixtures match $checkout_path @ $upstream_sha"
    else
        echo >&2
        echo "error: vendored content-encoding fixtures differ from $checkout_path @ $upstream_sha" >&2
...
staging="$(mktemp -d "$(dirname "$VENDOR_DIR")/.sync-content-encoding-fixtures.XXXXXX")"
previous=""
...
echo "  2. Run: cargo test -p tapesctl --test content_encoding_corpus"
...
echo "  3. If the decoder changed behaviour, land the fixture bump and the"
echo "     decoder change in the same PR — the corpus is the contract."
echo "  4. The authored home (tapes pkg/capture/contentencoding_corpus_test.go)"
echo "     must be green at the SAME SHA."
